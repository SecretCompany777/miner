...
██████╔╝███████╗╚█████╔╝██║  ██║███████╗   ██║
╚═════╝ ╚══════╝ ╚════╝ ╚═╝  ╚═╝╚══════╝   ╚═╝
[italic green]乂 W E L C O M E  T O  S E C R E T  S E R V I C E

          [italic white on red]Your IP Boss: {ip}""", style="bold bright_black", width=56))
  ...
  def getproxy(self):
    ...
    for dhttp in datahttp:
      ...
      for ddh in proxies:
        proxy.append(ddh)
    print("[bold bright_black]   ╰─>[bold green] Proxy updated successfully boss!", end='\r')
    time.sleep(3)
    ...
    for account in data:
      ...
      if is_login:
        if is_login["login"]:
          ...
          if execut["success"] == "1":
            print(Panel(f"""
[bold green]Balance : {balance}[/]
[bold green]Success : {self.count_sc(email)}[/]
[bold green]Failed : {self.count_fl(email)}[/]
[bold green]Claim : 0.{execut["key"]} XRP[/]
[bold green]Status :[/] [bold green]Success[/]""", style="bold bright_black", width=56, title=f">>> [bold green]{email}[/] <<<"))
...
if __name__ == '__main__':
  try:
    Xrpminer().logo()
    print(Panel(f"[italic blue]Currently updating proxy, tunggu sebentar...", style="bold bright_black", width=56, title=">>> Update Proxy <<<"))
    Xrpminer().getproxy()
    Xrpminer().logo()
    Xrpminer().run()
